...
export const getcategory = async(req, res)=>{
    try{
        let listaProducto = await Producto.find().populate('categoria',['categoria']).sort({categoria:-1})
        return res.send(listaProducto)
    }catch(err){
        console.error(err)
        return
    }
...
export const getquantity = async(req, res)=>{
    try{
        let listaProducto = await Producto.find().populate('categoria',['categoria']).sort({quantity:-1})
        return res.send(listaProducto)
    }catch(err){
        console.error(err)
        return
...
export const search = async(req, res)=>{
    try{
        ...
        console.log('El numero de productos es ', productos.length)
        if(productos.length == 0) return res.status(404).send({message: 'Producto not found'})
        return res.send({message: 'Productos found', productos})
    }catch(err){
        console.error(err)
        return res.status(500).send({message: 'Error searching productos'})
    }
}