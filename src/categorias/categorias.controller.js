...
export const save = async(req, res)=>{
    try{
        ...
        return res.send({message: 'Category save Successfull'})
    }catch(err){
        console.error(err)
        return res.status(500).send({message: 'Error saved Category', err})
    }
}
...
export const updateCat = async(req, res)=>{
    try{
        ...
        if(!updatedCategoria) return res.status(401).send({message: 'Category not found and not Updated'})
        return res.send({message: 'Updated category', updatedCategoria})
    }catch(err){
        console.error(err)
...
export const deleteCat = async(req, res)=>{
    try{
        ...
        if (!deleteCat) return res.status(404).send({message: 'Account not found and not deleted'})
        return res.send({message: `category ${deleteCat} deleted successfully`})
    }catch(err){
        console.error(err)
